...
function sec_session_start() {
    ...
    if (ini_set('session.use_only_cookies', 1) === FALSE) {
        header("Location: ?page=error&err=Could not initiate a safe session (ini_set)");
        exit();
    }
...
function login($email, $password, $mysqli) {
    ...
    if ($stmt = $mysqli->prepare("SELECT id, username, password, salt, access 
            ...
            if (checkbrute($user_id, $mysqli) == true) {
                ...
                if ($db_password == $password) {
                    ...
                    if (!$mysqli->query("INSERT INTO login_attempts(user_id, time) 
                                    VALUES ('$user_id', '$now')")) {
                        header("Location: ?page=error&err=Database error: login_attempts");
                        exit();
                    }
        ...
        header("Location: ?page=error&err=Database error: cannot prepare statement");
        exit();
    }
}
...
function check_add_element($val){
